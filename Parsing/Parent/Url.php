 ...
 * История версий:
 *
 * 1.11 (2009 декабрь 18)
 * Методы:
 *   deleteCommandWithPrefix - удаление комманды с указанным префиксом. Перенумерация результатов.
 *   getCommandNumberWithPrefix() - возврат комманды с префиксом.
 *   deleteCommand() - удаление комманды с именем или с номером. Перенумерация результатов.
 *   setRequestPathArray() - установка массива комманд для модификации.
 *   getRequestPathArray() - изменен. Возвращает массив комманд для модификации.
 * 
 * 1.10 (2009 декабрь 08)
 * Методы:
 *    getCommandStringFile - возврат строки комманд без прследнего слеша. Для файла.
 *    setCommandLast - установка последней комманды. Или первой, если нет их вообще.
 * 
 * 1.09 (2009 апрель 27)
 ...
 * Реализация интерфейса Countable - колличество комманд в строке.
 * 
...
//	           $this->_requestPath = substr($this->_requestString, 0, $this->_requestPathLength);
	...
	{
	    return $this->_requestPathArrayToModif;
	}

	/**
	 * Устанавливает массив, элементы которого части строки запроса, которые были разделена "/"
	 * В системе именуются командами.
	 *
	 * @return Dune_Parsing_Parent_Url
	 */
	public function setRequestPathArray($array)
	{
	    $this->_requestPathArrayToModif = $array;
	    return $this;
	...
	/**
	 * Возврат комманды с префиксом.
	 *
	 * @param string $string
	 */
	public function getCommandNumberWithPrefix($string)
	{
	    $number = 1;
	    $number_total = count($this->_requestPathArrayToModif);
	    $array = array();
	    foreach ($this->_requestPathArrayToModif as $key => $value)
	    {
	        if ((strpos($value, $string) === 0 or strpos($value, $string) === ''))
	        {
	            return $key;
	        }
	    }
	...
	/**
	 * Удаление комманды с префиксом. Перенумерация результатов.
	 *
	 * @param string $string
	 */
	public function deleteCommandWithPrefix($string)
	{
	    $number = 1;
	    $number_total = count($this->_requestPathArrayToModif);
	    $array = array();
	    foreach ($this->_requestPathArrayToModif as $value)
	    {
	        if (!(strpos($value, $string) === 0 or strpos($value, $string) === ''))
	        {
	            $array[$number] = $value;
	...
	/**
	 * Удаление комманды по имени. Перенумерация результатов.
	 *
	 * @param string $string
	 */
	public function deleteCommand($string, $with_number = false)
	{
	    $number = 1;
	    $array = array();
        if ($with_number)
        {
            unset($this->_requestPathArrayToModif[$string]);
        }
	    foreach ($this->_requestPathArrayToModif as $value)
	    {
	        if ($with_number or strcmp($value, $string))
	        {
	            $array[$number] = $value;
	            $number++;
	        }
	 ...
	 * @return string
	 */
	public function getCommandStringFile($sourse = false)
	{
	    if ($sourse)
	       $array = $this->_requestPathArray;
	    else 
	...
	/**
	 * Переустановка последней существующей eкомманды 
	 * Еслии комманд нет - устанавливает првую.
	 *
	 * @param mixed $name имя комманды
	 * @return Dune_Parsing_Parent_Url
	 */
	public function setCommandLast($name)
	{
	...
	{
	    return count($this->_requestPathArrayToModif);
	}
	...
	protected function _explodePath()
	{
	    if (!$this->_requestPathArrayExploded)
	    {
            ...
            $this->_requestPathArrayExploded = true;
