 ...
 * Dune Framework
 * 
 * Преобразовать строку на русском языке из неизвестной кодировки в кодировку windows-1251
 * 
 * Использовались набор функций автора:
 * (c) Yuri Popoff, Nov 2003, popoff.donetsk.ua
 * 
 * ----------------------------------------------------
 * | Библиотека: Dune                                  |
 * | Файл: Convert.php                                 |
 * | В библиотеке: Dune/String/Charset/Convert         |
 ...
 *
 * История версий:
 * 0.99 (2008 декабрь 26)
 * Класс пртестирован - работает. Имена методов не приведены к стандарту. Текст не отформатирован.
 *
 * 
 */

class Dune_String_Charset_Convert
{
    ...
    {
        $this->_stringSourse = $string;
    }
    
    
    /**
     * Возвращает строку конвертированную в windows-1251.
     *
     * @return string
     */
    public function getWindows1251()
    {
       $s = $this->_stringSourse;
    // returns a string converted from a best encoding (windows-1251 or koi-8r) to windows-1251
      $sa = $this->_charset_prepare($s);
      $s1 = $this->_charset_win_lowercase($sa);
      $r1 = 'windows-1251';
      ...
      $s2 = $this->_charset_win_lowercase($this->_charset_koi_win($sa));
      ...
      $s2 = $this->_charset_win_lowercase($this->_charset_utf8_win($sa));
...
//////////  Защищенные методы.    
    ...
    /**
     * Считает плохие символы в русском
     *
     * @param string $s
     * @return integer
     * @access private
    ...
    /**
     * Считает хорошие символы в русском 
     *
    ...
    protected function _charset_win_lowercase($s)
    {
      for($i=0;$i<strlen($s);$i++)
      {
        $c=ord($s[$i]);
