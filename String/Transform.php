 ...
 *
 * 0.97 (2009 октябрь 5)
 * При запуске setQuoteRussian() учитывается перевод строки и нотация html > и <
 * 
 * 0.96 (2009 сентябрь 23)
 * Обработка внутрених ссылок - добавление домена и протокола. Метод linkToBase()
 * 
 * 0.95 (2009 июнь 1)
...
class Dune_String_Transform
{
	...
	{
	    $needle = false;
	    if (strpos($this->_stringResult, "\r\n") !== false) // Текст набран в Win
	    {
	        $needle = "\r\n";
	    }
	    else if (strpos($this->_stringResult, "\n") !== false) // Текст набран в Unix
	    {
	        $needle = "\n";
	    }
	    else if (strpos($this->_stringResult, "\r") !== false) // Текст набран в Mac
	    {
	        $needle = "\r";
	    }
	...
	/**
	 * Добавление к внутренним ссылкам домена и протокола.
	 * @param string $domain домент без протокола
	 * @return Dune_String_Transform
	 *
	 */
	public function linkToBase($domain)
	{
	    $domain = 'http://' . $domain;
	    $this->_stringResult = preg_replace('!(<((img)|(a))[^>]+((src)|(href))=((")|(\')))/!', "$1" . $domain .'/', $this->_stringResult);	
	    return $this;
	}
	...
	{
	    $this->_stringResult = preg_replace("/(\S)&quot;([\s.,?!<]|$|(&lt;))/", '$1&raquo;$2', $this->_stringResult);	
