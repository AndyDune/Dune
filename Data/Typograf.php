<?php
/**
 * Типограф. Обработка текста.
 * 
 * По большей части доработанный код:
 ...
 * 
 * Данный PHP-класс позволяет автоматически типографировать тексты на стороне сервера.
 * 
 * Что может типограф:
 * — правильно расставлять кавычки и тире;
 * — удаляет «табы» и повторы пробелов;
 * — удаляет пробелы в начале и конце текста, а также пустые строки;
 * — удаляет пробелы перед знаками пунктуации, перед знаком %;
 ...
 * — ГРАД(23) на 23°;
 * — ДЮЙМ(15) на 15?;
 * — ЕВРО() на €, ФУНТ() на ?, ЦЕНТ() на ?;
 * — СТРВ() на ?, СТРН() на ?, СТРЛ() на ? СТРП() на ?, ВВОД() на ?;
 * — 12x18 на 12?18;
 * — http://example.ru на http://example.ru;
 * — (с) на ©, (r) на ™, +/- на ±.
 * 
 * Требования: PHP 5.* и библиотека iconv.
 * 
 * ----------------------------------------------------
 * | Библиотека: Dune                                  |
 * | Файл: Typograf.php                                |
 * | В библиотеке: Dune/Data/Typograf.php              |
...
class Dune_Data_Typograf
{
    ...
    const TYPOGRAF_MODE_NAMES = 1; // &mdash; - спец. символы в виде имён
    const TYPOGRAF_MODE_OLD = 2; // &#151; - спец. символы в виде устарев. кодов, для "гурманов"
	 ...
	 * @param integer $tmode используёте константы класса
