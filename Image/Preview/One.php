 ...
 * 
 * 0.96 (2008 июль 15)
 * Смена пропорций картинки-превью. возврат квадратной картинки. Недостаующие пиксели - белые.
 * Изменения в методе: getPreviewFileUrl($size = 100, $square = false)
 *
 * 0.95 (2008 июнь 26)
...
//        $path_preview = trim($path_preview, '/\\');
    ...
    /**
     * Удвалить все превьюшки картинки
     *
     */
    ...
    public function getPreviewFileUrl($size = 100, $square = false)
    {
        if (isset($this->_resultArray[$size]))
            return $this->_resultArray[$size];
        if ($this->_makePreview($size, $square))
        {
    ...
    * @access private
    */
    protected function _makePreview($size, $square = false)
    {
        ...
        if ($good)
        {
            ...
            else 
            {
    			...
    			{
    			    $thumbPic = new Dune_Image_Transform($original_pic);
//   				    $thumbPic = new Dune_Image_Thumbnail($original_pic);
    			    ...
    			    if ($square)
    			    {
    			        $thumbPic->setModeProportion('add');
    			        $thumbPic->changeProportion();
    			    }
