 ...
 * ----------------------------------------------------
 * | Библиотека: Dune                                  |
 * | Файл: ArrayFromPost.php                           |
 * | В библиотеке: Dune/Db/Collector/ArrayFromPost.php |
 ...
 * 
 */
class Dune_Db_Collector_ArrayFromPost implements ArrayAccess,Iterator
{
	...
	/**
	 * Возвращает сериализованный массив с зарегистрированными полями $_POST
	 * Необходима передача объекта mysqli или дочернего
	 * К сериализованному массиву применяется real_escape_string()
	 *
	 * @param mysqli $db
	...
	{
	    $this->_makeArrays();
	    return $db->real_escape_string(serialize($this->almostSet));
	}
	...
	{
	    $this->_makeArrays();
	    return mysql_real_escape_string(serialize($this->almostSet));
	}
